...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('quiz')
    .setDescription('Take a Naruto quiz for XP and Ryo!'),
  ...
  async execute(interaction) {
    ...
    if (!selected) {
      return interaction.reply({
        content: '🥲 Please select a character using `/select` first!',
        flags: 64
      });
    }
    ...
    if (!canTakeQuiz(userId)) {
      ...
      return interaction.reply({
        content: `⏱️ You can take the quiz again at ${next.toLocaleString()}.`,
        flags: 64
      });
    }
    ...
    const quizMsg = await interaction.fetchReply();
